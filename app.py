...
def load_dataframe(file) -> pd.DataFrame:
    ...
    for col in df.select_dtypes(include="object"):
        try:
            ...
            pass  # leave non‑numeric text untouched
    return df
...
def show_auto_insights(df: pd.DataFrame):
    """Quick, generic charts to spark strategic thinking."""
    with st.expander("🔍 Auto Insights & Charts", expanded=True):
        num_cols = df.select_dtypes("number").columns.tolist()
        # Correlation heat‑map if 2+ numeric cols
        if len(num_cols) > 1:
        ...
        # Simple totals by first few categorical columns
        if num_cols:
            target = num_cols[0]
...
st.title("📊 CSV Insight v8·4 – Auto‑Insights")
    ...
    if prompt:
        ...
        if msg.function_call:
            ...
            if func_name == "aggregate":
                ...
                # Simple bar chart for aggregates
